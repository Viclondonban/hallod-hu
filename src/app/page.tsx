...
import PodcastList from './podcast-list'; // Import the new client component
...
export default async function Home() {
  ...
  // 2. Fetch ALL active podcasts, sorted by newest
  const allPodcasts = await prisma.podcast.findMany({
    where: { isActive: true },
    orderBy: { createdAt: 'desc' },
    select: { // Select only the fields we need for the grid
      id: true,
      title: true,
      author: true,
      imageUrl: true,
  ...
  return (
    <main className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {/* Search Box and Podcast Grid (Client Component) */}
        <PodcastList 
          podcasts={allPodcasts} 
          featuredPodcastId={featuredPodcast?.id} 
        />

        {/* Empty State (Only if database is totally empty) */}
